...
% compute dE/d(net_out), i.e. derivitive of the error wrt the input to the output layer
...
%  {Data, Data_List} = generate_Data(Num_Clients, maps:new(), []),
 ...
 % Res   = server(Client_Pids, Data, Data_List, Model_Ws, N, StartTime, Stats),
  ...
  write_results("data/output" ++ integer_to_list(StartTime) ++ ".csv", Content).
